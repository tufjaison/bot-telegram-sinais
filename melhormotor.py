...
# -*- coding: utf-8 -*-
"""
Motor de Presunções Esportivas v4.3 – ALTA PROBABILIDADE DE APROVAÇÃO

PARÂMETROS AJUSTADOS PARA APROVAR MAIS APOSTAS:
  - EV mínimo: 3% (antes 8%)
  - Probabilidade mínima: 18% (antes 25%)
  - Odd mínima: 1.25 (antes 1.40)
  - Edge mínimo: 2% (antes 5%)
  - Volatilidade máxima: 25% (antes 15%)
  - Kelly fraction: 25% (antes 15%)
  - Score mínimo: 0.40 (antes 0.60)

Isso resulta em MAIS apostas aprovadas, porém com menor grau de confiança.

INSTRUÇÕES PARA RODAR COM PRIORIDADE MÁXIMA NO TERMUX:
  nice -n -20 python presuncao.py
  ou em segundo plano:
  termux-wake-lock
  nice -n -20 nohup python presuncao.py > log.txt 2>&1 &
"""
...
if not BZZ_API_KEY or not ODDS_API_KEY:
    raise ValueError("Faltam chaves no .env")

BANKROLL = 1000.0

# ------------------------------------------------------------
# PARÂMETROS DE RIGOR – CONFIGURADOS PARA ALTA PROBABILIDADE
# (valores mais baixos = mais apostas aprovadas)
# ------------------------------------------------------------
RIGOR = {
    'ev_minimo': 0.03,           # EV mínimo (3%) – antes 8%
    'prob_minima': 0.18,         # probabilidade mínima (18%) – antes 25%
    'odd_minima': 1.25,          # odd mínima (1.25) – antes 1.40
    'edge_minimo': 0.02,         # edge mínimo (2%) – antes 5%
    'volatilidade_maxima': 0.25, # volatilidade máxima (25%) – antes 15%
    'kelly_fraction': 0.25,      # fração do Kelly (25%) – antes 15%
    'score_minimo': 0.40,        # score mínimo (0.40) – antes 0.60
}
...
# ---------- MAIN ----------
def main():
    logger.info("🚀 Motor de Presunções v4.3 – ALTA PROBABILIDADE")
    ...
    if len(df_hist) > 50:
        ...
        logger.info(f"Backtest: RMSE casa={metrics['ensemble']['rmse_h']:.3f}, fora={metrics['ensemble']['rmse_a']:.3f}")
        if AVANCED['save_metrics']:
            with open('metricas_backtest.json', 'w') as f:
    ...
    if not eventos:
        logger.warning("Nenhum evento futuro.")
        return
    ...
    for evt in eventos:
        ...
        # Propensão
        prop_casa = df_filtrado[df_filtrado['home'] == casa]['hg'].mean() if not df_filtrado[df_filtrado['home'] == casa].empty else 1.2
        prop_fora = df_filtrado[df_filtrado['away'] == fora]['ag'].mean() if not df_filtrado[df_filtrado['away'] == fora].empty else 1.2
        prop_over = (df_filtrado['hg'] + df_filtrado['ag']).mean() / 2.5 if not df_filtrado.empty else 0.5
        prop_btts = df_filtrado[(df_filtrado['hg'] > 0) & (df_filtrado['ag'] > 0)].shape[0] / len(df_filtrado) if len(df_filtrado) > 0 else 0.5

        logger.info(f"\n--- {casa} x {fora} ({liga}) ---")
        logger.info(f"Presunção: {gh:.2f} – {ga:.2f}  |  IC80: [{ic_h_inf:.2f},{ic_h_sup:.2f}] – [{ic_a_inf:.2f},{ic_a_sup:.2f}]")
        logger.info(f"Propensão: Casa {prop_casa:.2f} | Fora {prop_fora:.2f} | Over {prop_over:.2f} | BTTS {prop_btts:.2f}")

        # Odds
        match_info = {'home_team': casa, 'away_team': fora, 'league': liga, 'date': data}
        odds = get_odds(match_info)
        if not odds:
            logger.warning("Sem odds – pulando")
            continue
        ...
        if aprov:
            ...
            logger.info(f"✅ APROVADA: {mapa[aprov['outcome']]} | Odd {aprov['odd']:.2f} | EV {aprov['ev']:.2%} | Stake R$ {aprov['stake_estimado']:.2f}")
            registrar(casa, fora, liga, '1X2', aprov['outcome'], aprov['odd'], aprov['stake_estimado'], aprov['ev'], aprov['score_percent'], data)
            total_aprovadas += 1
            aprov_1x2 = True
        else:
            logger.info("❌ Nenhuma 1X2 aprovada")
        ...
        if 'over' in odds and 'under' in odds:
            ...
            if aprov_ou_obj:
                logger.info(f"✅ APROVADA: {aprov_ou_obj['mercado']} | Odd {aprov_ou_obj['odd']:.2f} | EV {aprov_ou_obj['ev']:.2%} | Stake R$ {aprov_ou_obj['stake_estimado']:.2f}")
                ...
                logger.info("❌ Nenhum Over/Under aprovado")
        ...
        resultados.append({
            ...
            'fora': fora,
            'liga': liga,
            'gols_presuncao_casa': round(gh, 2),
            'gols_presuncao_fora': round(ga, 2),
            'ic_casa_inf': round(ic_h_inf, 2),
            'ic_casa_sup': round(ic_h_sup, 2),
            'ic_fora_inf': round(ic_a_inf, 2),
            'ic_fora_sup': round(ic_a_sup, 2),
            'propensao_casa': round(prop_casa, 2),
            'propensao_fora': round(prop_fora, 2),
            'propensao_over': round(prop_over, 2),
            'propensao_btts': round(prop_btts, 2),
            'prob_casa': round(probs_1x2.get('home', 0), 3),
            'prob_empate': round(probs_1x2.get('draw', 0), 3),
            'prob_fora': round(probs_1x2.get('away', 0), 3),
            'prob_over': round(p_over, 3),
            'prob_under': round(p_under, 3),
            'prob_btts': round(p_btts, 3),
            'odds_home': odds.get('home', 0),
            'odds_draw': odds.get('draw', 0),
            'odds_away': odds.get('away', 0),
            'odds_over': odds.get('over', 0),
            'odds_under': odds.get('under', 0),
            'volatilidade': round(volatilidade, 3),
            'aprovada': 'Sim' if (aprov_1x2 or aprov_ou) else 'Não'
        })

    # Exporta CSV
    if resultados:
        df_res = pd.DataFrame(resultados)
        df_res.to_csv('presuncoes_avancadas.csv', index=False, encoding='utf-8')
        logger.info(f"📊 CSV salvo: presuncoes_avancadas.csv ({len(resultados)} jogos)")
        if AVANCED['export_json']:
            exportar_json(resultados, 'presuncoes.json')
    else:
        logger.info("Nenhum dado gerado.")

    logger.info(f"✅ Total de apostas aprovadas: {total_aprovadas}")
