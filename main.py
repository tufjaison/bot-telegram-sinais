...
# --- SERVIDOR FAKE PARA MANTER O RENDER ONLINE (PORTA WEB) ---
web_app = Flask(__name__)

@web_app.route('/')
def home():
    return "Bot de Sinais está Online e Rodando!"

def run_web_server():
    port = int(os.environ.get("PORT", 10000))
    web_app.run(host="0.0.0.0", port=port)

# --- CONFIGURAÇÕES DO BOT TELEGRAM E API ---
TELEGRAM_BOT_TOKEN = "8841800451:AAE_4-wSQ8LDY-uJH9s09uulW8S9_DDvSlo"
CHAT_ID = "-1004348164311"  # <--- SEU CHAT_ID COM SINAL DE MENOS (-)
...
async def consultar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for partida in jogos_encontrados:
        ...
        if liga_id in LIGAS_ALVO:
            jogos_processados += 1
            # O limite de 90 foi removido: agora analisa TODOS os jogos do dia
...
def main():
    # Inicia o servidor Web no fundo para agradar o Render
